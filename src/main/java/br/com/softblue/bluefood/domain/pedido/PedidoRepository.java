...
public interface PedidoRepository extends JpaRepository<Pedido, Integer> {
	...
	@Query("SELECT i.itemCardapio.nome, SUM(i.quantidade), SUM(i.preco * i.quantidade) FROM"
			+ " Pedido p INNER JOIN p.itens i WHERE p.restaurante.id = ?1 AND i.itemCardapio.id = ?2 AND p.data BETWEEN ?3 AND ?4"
	...
	@Query("SELECT i.itemCardapio.nome, SUM(i.quantidade), SUM(i.preco * i.quantidade) FROM"
			+ " Pedido p INNER JOIN p.itens i WHERE p.restaurante.id = ?1 AND p.data BETWEEN ?2 AND ?3"
			+ " GROUP BY i.itemCardapio.nome")
